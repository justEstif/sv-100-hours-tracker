            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {#each commitments as commitment, i (commitment.id)}
                {@const progressPercent = Math.min(
                  100,
                  Math.round(
                    (commitment.totalMinutes / 60 / commitment.goalHours) * 100,
                  ),
                )}
                <a
                  href="/{commitment.id}"
              ...
              <form {...createCommitment} class="space-y-4">
                ...
                <fieldset class="fieldset">
                  <legend class="fieldset-legend"
                    >What do you want to master?</legend
                  >
                  <input
                    {...createCommitment.fields.title.as("text")}
                    class="input w-full"
                    class:input-error={(createCommitment.fields.title.issues()
                      ?.length ?? 0) > 0}
                ...
                <fieldset class="fieldset">
                  ...
                  <input
                    ...
                    class="input w-full"
                    class:input-error={(createCommitment.fields.category.issues()
                      ?.length ?? 0) > 0}
                ...
                <button
                  type="submit"
                  disabled={!!createCommitment.pending}
